namespace AnimDataNS
{
    ...
    public class AnimDataPlayer : MonoBehaviour
    {
        ...
        private void OnValidate()
        {
            ...
            if( animationData != null && ( binds == null || binds.link != animationData ) )
            {
            ...
            {
                bool zero_bind = false; // temp check , shouldn't be needed ..

                bool missing_target = false; // why ? 

                foreach( var bind in binds.data )
                {
                    if ( bind.target == null || bind.data == null ) continue;

                    if( ( zero_bind &= bind.data.count < 1 ) ) continue;

                    var t = bind.data.TotalTime() * time; 
                
                    if( bind.target == null ) missing_target = true;
                ...
                if( zero_bind ) Debug.LogWarning("TODO: fix, found zero bind , temp fix : click [Regenerate binds]");

                if( missing_target ) Debug.Log("TODO: fix, missing target reference , temp fix : click [Try auto bind]");
            }
        }
        ...
        class Inspector : Editor
        {
            public override void OnInspectorGUI()
            {
                ...
                {
                    if( script.binds.data[0].data.count == 0 )
                    {
                        if( GUILayout.Button("Regenerate binds") )
                        {
                            script.binds = null;
                        }
                    }
